...
public class FindbugsConfiguration {
	...
	public FindbugsConfiguration(FileSystem fileSystem, Configuration config, RulesProfile profile,
	...
	public Project getFindbugsProject() throws IOException {
		...
		if (annotationsLib != null) {
			// Findbugs dependencies are packaged by Maven. They are not available during
	...
	/**
	 * Return the complete list of Java files from the project (excluding test
	 * files)
	 * 
	...
	private Iterable<File> getSourceFiles() {
		...
		return fileSystem.files(
				pred.and(pred.hasType(Type.MAIN), pred.or(FindbugsPlugin.getSupportedLanguagesFilePredicate(pred)),
						pred.not(pred.matchesPathPattern("**/package-info.java")),
						pred.not(pred.matchesPathPattern("**/module-info.java"))));
	}

	/**
	 * Creates a class screener to filter the files for analysis by findbugs. The
	 * filter is based on sonar.findbugs.onlyAnalyze {@link FindbugsConstants}
	 * property
	 * 
	 * @return ClassScreener object if property is present and not empty, null
	 *         otherwise.
	 */
	@VisibleForTesting
	...
	/**
	 * Determine if the project has Java source files. This is used to determine if
	 * the project has no compiled classes on purpose or because the compilation was
	 * omit from the process.
	 * 
	...
	/**
	 * Scan the given folder for classes. It will catch classes from Java, JSP and
	 * more.
	 *
	...
	public String getEffort() {
		return StringUtils.lowerCase(
				config.get(FindbugsConstants.EFFORT_PROPERTY).orElse(FindbugsConstants.EFFORT_DEFAULT_VALUE));
	}

	public String getConfidenceLevel() {
		return StringUtils.lowerCase(config.get(FindbugsConstants.CONFIDENCE_LEVEL_PROPERTY)
	...
	public boolean isAllowUncompiledCode() {
		return config.getBoolean(FindbugsConstants.ALLOW_UNCOMPILED_CODE)
				.orElse(FindbugsConstants.ALLOW_UNCOMPILED_CODE_VALUE);
	}
	...
	public static List<PropertyDefinition> getPropertyDefinitions() {
		...
		return ImmutableList.of(PropertyDefinition.builder(FindbugsConstants.EFFORT_PROPERTY)
				...
				PropertyDefinition.builder(FindbugsConstants.ALLOW_UNCOMPILED_CODE)
						...
						.type(PropertyType.BOOLEAN).category(CoreProperties.CATEGORY_JAVA).subCategory(subCategory)
						...
						.onQualifiers(Qualifiers.PROJECT, Qualifiers.MODULE).build(),
				PropertyDefinition.builder(FindbugsConstants.REPORT_PATHS).category(CoreProperties.CATEGORY_JAVA)
						.subCategory(subCategory).name("Report Paths")
						.description(
								"Relative path to SpotBugs report files intended to be reused. (<code>/target/findbugsXml.xml</code> and <code>/target/spotbugsXml.xml</code> are included by default)")
						.onQualifiers(Qualifiers.PROJECT, Qualifiers.MODULE).multiValues(true).build());
	}
